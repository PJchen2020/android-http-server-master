...
import ro.polak.http.ServerRunnable;
...
public class HttpServletRequestImplFactory {
    ...
    //derek debug
    private static final Logger LOGGER = Logger.getLogger(HttpServletRequestImplFactory.class.getName());
    ...
    public HttpServletRequestImpl createFromSocket(final Socket socket)
        ...
        LOGGER.log(Level.INFO, "getRemoteSocketAddress = " + socket.getRemoteSocketAddress().toString());
        //LOGGER.log(Level.INFO, "isInputShutdown = " + socket.isInputShutdown());

        InputStream in = socket.getInputStream();
        //LOGGER.log(Level.INFO, "InputStream in = socket.getInputStream(), in object = " + in.toString());
        //LOGGER.log(Level.INFO, "isInputShutdown = " + socket.isInputShutdown());
        // The order matters
    ...
    private String getStatusLine(final InputStream in)
        ...
        //LOGGER.log(Level.INFO, "start getStatusLine func --> " + in.toString());
        StringBuilder statusLine = new StringBuilder();
        LOGGER.log(Level.INFO, "available bytes from InputStream --> " + in.available());

        //add by Derek to fix the issue of
        //"if available bytes from InputStream --> 0, then will block in in.read(buffer, 0, buffer.length)"
        //move to ServerRunnable.run to avoid this issue early
//        if(0 == in.available()) {
//            in.close(); //close the socket
//            return "Get /admin/Empty HTTP/1.1";
//        }
        ...
        while (in.read(buffer, 0, buffer.length) != -1) {
            ...
            LOGGER.log(Level.INFO, "statusLine.append --> " + (char)buffer[0]);
        ...
        //LOGGER.log(Level.INFO, "finished getStatusLine func, result is: " + statusLine.toString());
